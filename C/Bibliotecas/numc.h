...
funções*/
int validaDimensaoInt(int dimensao);
...
/* validaDimensaoReal() - Função para evitar a inserção de dimensoes negativas nas
funções*/
float validaDimensaoReal(float dimensao);
...
de acordo com as dimensões passadas. Não necessita, obrigatoriamente, ser quadrada.*/
float** matrizOnesInt(unsigned short int linha, unsigned short int coluna);
...
/* Calcula o determinante de uma matriz quadrada*/
float determinanteInt(unsigned short int linha, unsigned short int coluna,
                     float** matriz);
